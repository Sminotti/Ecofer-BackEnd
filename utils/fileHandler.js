...
import { unlink, createReadStream, createWriteStream } from "fs"; // Propio de nodejs// lee el archivo temporal
import { v4 as uuid } from "uuid";// renombro al archivo con un nombre unico
const allowExtension = ["png", "jpeg", "jpg"];// extenciones permitidas
// mimetype

const deleteTemp = (file) => unlink(file, (e) => console.log(e));//variable que borra el temporal//fs.unlink(archivo),permite manejar el archivo
// mimetype
//gurada el archivo
...
const saveFile = ({ mimetype, size, path },allowE,destFolder = `../FrontEnd/src/assets/images`) => {
  try {
    ...
    createReadStream(path).pipe(createWriteStream(fileNameOut));//leo el temporal de la ruta (path) y lo creo en una imagen
