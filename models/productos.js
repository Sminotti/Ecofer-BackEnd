...
const update = (obj, id) =>
  pool
    .query("UPDATE ?? JOIN ?? ON productos.idCategoria = categoriaproductos.id JOIN ?? ON productos.idProveedor = proveedores.id JOIN ?? ON productosimagen.idProductos = productos.id SET ? where productos.id = ?",
...
const updateImage = (obj, id) =>
  pool
    .query("UPDATE ?? SET ? where productosimagen.idProductos = ?", [T_PRODUCTOS_IMAGENES, obj, id])
    .then((response) => response)
    .catch((e) =>console.log(e));
